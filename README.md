...
docs/                         # GitHub Pagesの公開ルート
  ...
  data/YYYY-MM.ndjson         # 月ごとの生データ（1行1レコードで追記）
  data/summary.json           # 1時間毎に集約した時系列（長期グラフ用・保持90日）
...
**Actions** タブから `Fetch SwitchBot Sensor Data` を `workflow_dispatch` で手動実行し、`docs/data/latest.json`・`docs/data/YYYY-MM.ndjson`・`docs/data/summary.json` が更新・コミットされることを確認します。以降は15分毎に自動実行されます。
